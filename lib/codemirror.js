define([
  ...
  exports.attach = function attach() {
    ...
    CodeMirror.prototype.stripEmpty = function stripEmpty(cm) {
      cm.eachLine(function(line) {
        if (line.text.match(/^\s+$/))
          line.text = "";
      });
    };
    CodeMirror.prototype.removeOverallIndentation = function removeOverallIndentation(cm) {
      var overall_indent = Infinity;
      cm.eachLine(function(line) {
        var indent = line.text.search(/\S|$/);
        if (indent != line.text.length && overall_indent > indent)
          overall_indent = indent;
      });

      if (isFinite(overall_indent) && overall_indent > 0) {
        var pos = cm.getCursor();
        cm.eachLine(function(line) {
          line.text = line.text.substring(overall_indent);
        });
        cm.setValue(cm.getValue());  // force refresh
        cm.setCursor(pos.line, pos.ch - overall_indent); // restore cursor
      }
    };
    CodeMirror.prototype.handleCtrlC = function handleCtrlC(cm) {
      var cell = ns.notebook.get_selected_cell();

      if (cell && cm && cm.state.vim.insertMode) {
        CodeMirror.prototype.stripEmpty(cm);
        if (cell.cell_type == 'code') {
          CodeMirror.prototype.removeOverallIndentation(cm);
        }
        CodeMirror.Vim.handleKey(cm || this, '<Esc>');
      } else if (cell && cm && cm.state.vim.visualMode) {
