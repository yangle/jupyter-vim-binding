...
define([
  ...
  exports.attach = function(params) {
    ...
    actions.register({
      'help': 'insert cell below and enter Vim insert mode',
      'handler': function(env, event) {
        env.notebook.command_mode();
        ...
        CodeMirror.Vim.handleKey(cell.code_mirror, 'i');
      }
    }, 'insert-cell-below', 'vim-binding'); // 'vim-binding:', not 'jupyter-notebook:'

    actions.register({
      'help': 'insert cell above and enter Vim insert mode',
      'handler': function(env, event) {
        env.notebook.command_mode();
        var actions = Jupyter.keyboard_manager.actions;
        actions.call('jupyter-notebook:insert-cell-above', event, env);
        env.notebook.edit_mode();

        var cell = Jupyter.notebook.get_selected_cell();
        CodeMirror.Vim.handleKey(cell.code_mirror, 'i');
      }
    }, 'insert-cell-above', 'vim-binding'); // 'vim-binding:', not 'jupyter-notebook:'
  };
